...
**认领并执行 `GWP-010`：锁定完整UI体验规格。**

本任务需要先完成24类屏幕、状态矩阵、用户流程、真实中文文案、组件清单和图像资产需求，并由用户确认，之后才能进入gpt-image-2视觉方向探索。
...
`GWP-010`当前为`READY`；`GWP-011`至`GWP-026`保持`BLOCKED`，按上述顺序逐一解锁。所有Figma写任务`parallel_safe: false`，禁止并行修改设计文件。
