...
**D2：Figma工作区发现、范围锁定与状态台账**

项目已纠正为设计先行顺序。正式路线为：完整界面规格 → gpt-image-2视觉基准 → Figma设计系统 → 24类界面、状态、适配、原型和审计 → Figma设计冻结 → Cocos工程与玩法。

## 下一动作

**认领并执行 `GWP-013`：Figma工作区发现、范围锁定与状态台账。**

使用已批准的B“充气贴纸工坊”视觉基准，先只读发现目标Figma文件、共享Page、可用库、变量、组件、样式、字体和编辑边界；随后建立唯一顶层Section、gap analysis与可跨窗口恢复的`STATE.json`。本任务不得创建变量、组件、正式屏幕或修改Cocos工程。
...
`GWP-010`至`GWP-012`已`DONE`，`GWP-013`当前为`READY`；`GWP-014`至`GWP-026`保持`BLOCKED`，按上述顺序逐一解锁。所有Figma写任务`parallel_safe: false`，禁止并行修改设计文件。
...
- Figma项目Section、gap analysis、变量、组件、屏幕和状态台账尚未创建；`GWP-013`必须先锁定编辑边界与v1设计系统范围。
