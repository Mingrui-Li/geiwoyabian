...
合入后将`GWP-009`置为`DONE`并解锁`GWP-010`。在此之前不得认领后续设计或Cocos任务。

## 活跃任务

当前无`IN_PROGRESS`任务；`GWP-009`处于`REVIEW`。

## READY

当前无READY任务，等待`GWP-009`集成。
...
`GWP-010`至`GWP-026`当前均为`BLOCKED`，按上述顺序逐一解锁。所有Figma写任务`parallel_safe: false`，禁止并行修改设计文件。
...
- 24类界面目前只有清单，没有逐屏内容、真实文案和状态规格；这是`GWP-010`必须解决的问题。
- 正式视觉方向尚未生成或由用户选择。
...
- 集成分支：`main`
- 当前修正规划分支：`codex/ui-first-planning-fix`
