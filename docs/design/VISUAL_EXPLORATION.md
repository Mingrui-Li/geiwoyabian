...
用户于2026-08-27明确选择B“充气贴纸工坊”为唯一主方向。A与C保留为历史候选，不进入批准资产；B当前仍是候选视觉输入，必须由`GWP-012`完成规范、修正和资产批准后才能进入Figma。

固化时必须：

1. 保留B的白色贴纸衬边、深墨蓝模切外框、轻偏移阴影和大色块优势。
2. 分别定义脆性、弹性、爆浆、嵌套和不稳定物品的表面与变形规则，避免全部使用同一种缝线气垫表现。
3. 将主入口、游戏舞台和结算场景批准基准统一为正式竖屏构图；B结果候选的等比留白不能成为最终版式。
4. 若需要吸收A或C的局部特征，必须由用户明确列出且不超过两项，禁止整套混用。
